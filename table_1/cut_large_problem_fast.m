...
% pointball_fv=zeros(n_outer,1);
% pointball_vio=zeros(n_outer,1);


% con_num_elli=zeros(n_outer,1);
% max_eig_elli=zeros(n_outer,1);
% bt_mean_max_eig_elli=zeros(n_outer,1);
% bt_std_max_eig_elli=zeros(n_outer,1);
...
%         sample_cov_1=((cov(dataset_p1_1)))+eye(m*n)*1;
...
%         sample_cov_2=diag(diag(cov(dataset_p1_1)));
        ...
        % using the phase I data to find a good cut.
        mean_mat_b1_for_scale=repmat(sample_mean_1,B_1,1);

%         con_num_elli(i)=cond(sample_cov_1);
%         max_eig_elli(i)=max(eig(sample_cov_1));
%         %
%         bt_eig=zeros(20,1);
%         for j_bt=1:20
%             rand_indx=randsample(1:B_1,B_1,true);
%             bt_cov=cov(dataset_p1_1(rand_indx,:));
%             bt_eig(j_bt)=max(eig(bt_cov));
%         end
%         bt_mean_max_eig_elli(i)=mean(bt_eig);
%         bt_std_max_eig_elli(i)=std(bt_eig);
%         
        ...
        % find the quantile
        % inverse of binomial cumulative dist fun X = binoinv(Y,N,P)
        s1_1_for_solve_x0=sorted_t_for_cut(rank_of_data_p1+1);
       % covers halp of the data
 %% optimization for cut slope
       ...
       M_total_scb=sqrtm(sample_cov_2);
       M_spl_scb=zeros(M_r,A1_c,A1_r);
       for j=1:A1_r
         M_spl_scb(:,:,j)=M_total_scb(:,(j-1)*A1_c+1:j*A1_c);
       end
 ...
 %%     find cut point
       ...
       in_elli_cut_data_index=t_value_b1_for_cut<=s1_1_for_cut;
       in_elli_cut_data=dataset_p1_1(in_elli_cut_data_index,:);
       [srted_cut_line,sort_index_cut_line]=sort(in_elli_cut_data*x0_for_cut);
       cut_point=sort_index_cut_line(ceil(sum(in_elli_cut_data_index)*0.95));
     
       % cut line  x0'*a-x0'*sample_mean_1<= (x0' *
       % in_elli_cut_data(cut_point,:)'-x0'*sample_mean_1)
%        x0_for_cut'*(a-sample_mean_1')/(x0_for_cut' *in_elli_cut_data(cut_point,:)'-x0_for_cut'*sample_mean_1')<= 1
 
%% define cut line and find cut line t
 ...
 %% phase ii      
       T_value_b2_elli=(dataset_p2_1-mean_mat_b2_1)*sample_cov_1^-1*(dataset_p2_1-mean_mat_b2_1)'/s1_1_for_cut;  % calculate only for diag
      ...
      [sorted_t_old,sort_indexold]=sort(t_value_b2_elli);
        ...
        [sorted_t_scb,sort_indexscb]=sort(t_value_b2_scb);
        s_scb=sorted_t_scb(rank_of_data+1);
...
% n=2 here
         ...
         [ cvx_status1 f_val_old x_elli_old] = cvx_elli_LP_one_line( c,(s_old)^2*s1_1_for_cut,M_spl,A_m,b1,n);   
